        ...
        while (flag)
        {
            ...
            if (buffer[i]=='"')
            {
                ...
                else if (buffer[i + 1]==',' || buffer[i + 1]=='\n' || buffer[i + 1]=='\0')
                {
                    ...
                    throw "Ops, parece que o arquivo está fora do padrão especificado.";
            }
            i++;
        }
    }
...
{
    //cout<<"ENTROU EM IMPORTA QUESTOES"<<endl;
    ...
    if (arquivo.is_open())
    {
        ...
        //criando um vetor de char com o arquivo dentro
        ...
        while (n < numQuestoes)
        {
            ...
            if (elemento=="NA")   //existem usuários que não tem identificação
            {
                ...
                vetorQuestoes[n].setUser(converteEmNumero('0' + elemento));     //peguei o UserID}
            i = i + elemento.length() + 1;
...
{
    //cout<<"ENTROU EM VETOR DE BODYS"<<endl;
    //importa somente os IDs das questoes como INT
    string *bodys = new string[numQuestoes];                     //vetor para retornar
    question *vetorQuestoes = importaQuestoes(numQuestoes);      //chama a outra função
    //cout<<"SAIU DE IMPORTA QUESTOES"<<endl;

    //pega só os IDs
    //cout<<"ENTROU EM FOR"<<endl;
    for (int q = 0; q < numQuestoes; q++)
    {
        bodys[q] = vetorQuestoes[q].getBody();
        //cout << q << " " << ids[q] << endl;                    //descomente para acompanhar a importação
    }
    //cout<<"SAIU DE FOR"<<endl;
    //delete vetorQuestoes;
    //cout<<"SAIU DE VETOR DE BODYS"<<endl;
    return bodys;
}
...
void controlaArquivo::imprimeSaida(string nome, int tamanho, double tempoGasto, long long unsigned tamanhoDoArquivoEmBytes)
{
    ...
    if (saida.is_open())
    {
        ...
        cout << "Nao consegui abrir o arquivo saida.txt";
}
...
void controlaArquivo::imprimeBodysBinarios(string *vetorDeBodys, int tamanho, string nomeDoArquivo)
{
    nomeDoArquivo += ".bin";
    fstream arquivo;
    arquivo.open(nomeDoArquivo, std::ofstream::out | std::ofstream::trunc | ios::binary);
    if (arquivo.is_open())
    {
        int i = 0;
        while (i < tamanho)
